...
class SubmitPublicQuotationCustomerDto {
  @ApiProperty({
    description: 'Modo de registro del cliente',
    enum: ['new', 'existing'],
    enumName: 'CustomerMode',
    example: 'new',
  })
  @IsIn(['new', 'existing'])
  mode!: 'new' | 'existing';
  ...
  @ApiPropertyOptional({
    ...
    example: 'Jr. Comercio 456, Arequipa',
  })
  @IsOptional()
  @IsString()
  @MaxLength(1000)
...
export class SubmitPublicQuotationDto {
  @ApiProperty({
    description: 'Datos del cliente que realiza la cotización',
  ...
  @ApiProperty({
    description: 'Lista de productos/servicios solicitados',
  ...
  @ApiPropertyOptional({
    ...
    example: 'Cotización para evento escolar',
  })
  @IsOptional()
  @IsString()
  @MaxLength(2000)
  ...
  @ApiPropertyOptional({
    description: 'Token de verificación Turnstile (Cloudflare)',
