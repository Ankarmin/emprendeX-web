...
describe('PublicCatalogExperience', () => {
  ...
  it('adds items and opens the cart', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Agregar Café Especial' }));

    expect(screen.getByRole('button', { name: 'Aumentar Café Especial' })).toBeInTheDocument();
    expect(screen.getAllByText('1')).toHaveLength(2);

    await user.click(screen.getByRole('button', { name: 'Abrir carrito' }));

    expect(screen.getByRole('heading', { name: 'Tu cotización' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Confirmar cotización' })).toBeEnabled();
  });
  ...
  it('normalizes API field variants before filtering and adding items', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Agregar Miel Artesanal' }));
    await user.click(screen.getByRole('button', { name: 'Abrir carrito' }));
    ...
    expect(
      within(cartHeading.closest('aside') ?? document.body).getByText('Miel Artesanal'),
    ).toBeInTheDocument();
  });
});
